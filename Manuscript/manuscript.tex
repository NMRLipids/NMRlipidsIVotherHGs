...
Experimental order parameters for bilayers with PS headgroups are 
available in literature only for DOPS  \cite{browning80}. These are shown together
with the results from different simulation models in Fig. \ref{HGorderParametersDOPS}.
The signs are not yet know experimentally.
Before the experimental data becomes available we have denoted 
negative signs for $\beta$ and $\alpha$ segment order parameters,
because this selection of signs is in best agreement with the most simulation
results. With this interpretation the CHARMM36, CHARMM36ua and Slipids
models would be in relatively good agreement with experimental data in
the headgroup region. However, if the experimental signs turn out to
be different than expected, this conclusion will change.
\begin{figure}[]
  \centering
  \includegraphics[width=9.0cm]{../Figs/HGorderparametersDOPS.eps}
  \caption{\label{HGorderParametersDOPS}
    ...
    Signs are not known experimentally. They chosen to give best agreement simulations.
  }
  \todo{Experimental signs of the order parameters would highly useful.} \\
\end{figure}

The glycerol backbone order parameters for Slipids model are, however,
far from experimental values, as previously observed also for PC 
lipids \cite{botan15}. The glycerol backbone structures between PC and
...
parameter of bound charge \todo{Should we check this against cationic surfactant experiments \cite{scherer89}?}.
\begin{figure}[]
  \centering
