...
The simulations were maintained at temperatures of 303 K and 298 K for DOPS and POPS respectively using the Nosé-Hoover thermostat \cite{nose84,hoover85}
...
switched off between 1.0 nm and 1.2 nm; Coulombic interactions were truncated at 1.2 nm with long-range interactions treating using PME~\cite{darden93,essman95}.
...
Coulombic interactions were truncated at 1.0 nm with PME used to treat interactions beyond this cut-off.
...
\noindent
{\it POPC:POPS (5:1) mixtures with additional calcium chloride concentration} \\
...
One set of simulations has been performed using Amber18 simulation package \cite{amber18md} and ff99SB~\cite{aqvist90} ion parameters. 
Number of ions and water molecules for this set of simulations are shown in Table~\ref{mixedIONsystems}. The simulation details, minimization, heating, and equilibration runs are the same as described above for the other POPC:POPS (5:1) simulations with Amber Lipid17 forcefield. 
For each ion concentration, 100~ns equilibration runs were followed by 300~ns production runs with two independent trajectories, of which last 200~ns were used for the analysis. The trajectory files can be found in Ref~\citenum{POPCpopsLIPID17withCaCl} 

Another set of simulations with Lipid17 force field for lipids \cite{gould18}
and Dang parameters for ions \cite{smith94, dang06}
was performed using the GROMACS 2018 simulation package~\cite{abraham2015gromacs}. 
We used the time step of 2~fs. 
...
The composition of the individual simulations is shown in Table~\ref{mixedIONsystems}. 
Initial structures obtained from CHARMM-GUI~\cite{lee16,jo18} 
were equilibrated prior to production simulations used for analysis,
...
for the CHARMM36 simulations. Force field parameters were taken from the published Slipids PS model,
...
and POPS simulations bar, including the use of the Verlet cut-off scheme, apart from all bonds were constrained using LINCS in these simulations.
...
\noindent
{\it Hanne Antila}
Contiributed to the development analysis tools used for evaluating the force fields,  provided critical discussion on the manuscript content and edited it for clarity.

\noindent
{\it Pavel Buslaev}
Analyzed the dihedral angle distributions in the head group and glycerol backbone regions.
...
\noindent
{\it Thomas Piggot}
Setup, performed and analysed many of the simulations. Contributed to parts of the manuscript.
