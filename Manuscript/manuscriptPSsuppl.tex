...
Inputs for simulations of POPC:POPS (5:1) mixtures containing a total of 132 lipids (110 POPC and 22 POPS) were configured using the CHARMM-GUI~\cite{lee16,jo18}. Two identical systems were constructed, apart from the counter ion; one system was neutralised through the addition of 22 Na$^+$ ions while the other system
contained 22 K$^+$ ions. As in the pure POPS CHARMM36 simulations, 35 water per lipid were added. All other system and simulation parameters
...
POPC/POPS mixtures with the MacRog force field, see below. The simulation was run for 200~ns using Gromacs 2016.3~\cite{abraham2015gromacs}, 
...
including use of the Verlet cut-off scheme~\cite{Pall13}, except that here LINCS was used to constrain all bonds.
...
each system, 128 NaCl molecules were added to neutralize the system.
...
to constrain the bonds in TIP3P water molecules. Simulations were run using Amber18 simulation package \cite{amber18md}
...
ion parameters~\cite{joung2008determination} and another set of simulations were run with ff99SB (\AA{}qvist)~\cite{aqvist90} ion parameters.
...
algorithms while restraining the solvent and ions with a force
...
\noindent
{\it POPC:POPS (5:1) mixtures with additional calcium chloride concentration using default Amber parameters.} 
...
The simulation details, minimization, heating, and equilibration runs were the same as described above for the other POPC:POPS (5:1) simulations with Amber Lipid17 forcefield. 
...
\noindent
{\it POPC:POPS (5:1) mixtures with additional calcium chloride concentration using Dang ion parameters.} 
...
it is important to note that these $S_\mathrm{CH}$s
