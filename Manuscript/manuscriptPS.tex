...
\todo{Maybe we should add a figure in the SI.}
...
when comparing the PS results to PC (Fig. 2 in Ref.~\citenum{botan15} vs. Fig. \ref{HGorderParametersPS}).
...
\todo{We should consider making the conformation figures from CHARMM36 and Slipids POPC, and compare these
  to the results from POPS.}

\todo{Maybe we should move the glycerol backbone results to SI and focus here on the headgroup,
and especially to the difference between PC and PS.}
...
This suggests that the low area per molecule in PS lipid bilayers cannot be
explained only by counterion binding but also the hydrogen bonding network has to
...
Previously used method to evaluate counterion binding affinity to lipid bilayers
using the changes of hedgroup order parameters as a function
additional ions \cite{catte16} is difficult to apply for counterions because
reference state without ions in solution does not exist, see section \ref{mixtureTOadditionalCIs}
in supplementary information. However, the counterion binding affinity is
...
%The headgroup order parameters of POPC increase
%with increasing amount of negatively charged POPS
The experimentally measured order parameter increase~\cite{scherer87}
is reproduced in simulations with MacRog and potassium counterions, but not in 
other simulations (Fig. \ref{HGorderparametersPCvsPS}). 
The increase of POPC headgroup order parameters with increasing amount of POPS 
can be explained using the electrometer concept \cite{seelig87,scherer89}:
The PC headgroup tilts more parallel to the membrane plane with increasing
amount of negative charge in the membrane. In Berger simulations,
the strong sodium binding surpasses the effect of negatively charged PS
lipids and the order parameters actually decrease because also the amount of
positive counterions increase with increasing amount of PS.
...
of PS. Therefore, we conclude that the weak binding of potassium in the MacRog
simulations (Fig. \ref{NAdensPOPS}) leads to the correct surface charge
density and the response of POPC headgroup to the increasing amount of negatively
charges PS lipids in the lipid mixtures. 
\begin{figure*}[]
  \centering
