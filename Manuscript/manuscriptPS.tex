...
owing to electrostatic interactions or the formation of a hydrogen bonding network between the headgroups~\cite{browning80,buldt81}.
...
in order to evaluate the quality of the simulation model and/or to interpret the experiments \cite{ollila16}. Using this approach,
...
as previously observed also for PC lipids \cite{botan15}. Moreover, none of the models produces a set of order parameters in full quantitative agreement with the experiments.
Generally, the models perform less well for PS than for PC
(Figs.~\ref{HGorderParametersPS} and \ref{comparisonTablePS} vs. Figs.~2 and 4 in Ref.~\cite{botan15}).
This complicates the interpretation of structural differences between the PC and PS headgroups. However, concentrating on the headgroup alone, we see that the best performing models (Slipids, CHARMM36 and CHARMM36-UA) indeed do replicate the larger-than-in-PC
forking of the $\alpha$-carbon that is observed in the experiments. Additionally, the Slipids force field correctly produces the significantly smaller $\beta$-carbon order parameter for PS compared to PC (Fig. \ref{HGorderParametersPS} vs. Fig. 2 in Ref.~\citenum{botan15}), in line with
...
\begin{figure}[!tb]
  ...
  \caption{\label{HGorderparametersPCvsPS}
    ...
    % {\color{red} Why divided by two?}
    % Because then both values fit within the error bar.
  }
\end{figure}
...
The results are consistent with the previous study suggesting that the
...
% Would be potentially interesting to flesh out what the role of the hydrating water molecules play in the
% specific lipid headgroup/ion configurations that are sampled in simulations. My feeling is that "fully hydrated"
% lipids might carry with them differing amounts of water between the different FFs.. -JJM
...
Experimentally, the both headgroup order parameters of POPC
